...
## 补充三 · 自动开盘执行（替代“人工开盘窗口”）

**背景**：open 窗口设 15 分钟偏长——今天东鹏开盘 15 分钟内已大涨，人工点容易错过/吃波动。改为**开盘瞬间由机器自动触发**。

**规则**（`portfolio_risk.auto_open_execute`，默认关）：
```yaml
auto_open_execute:
  enabled: false
  delay_sec: 60     # 9:30 后 60 秒触发（留集合竞价→连续竞价过渡）
  grace_min: 25     # 触发点后允许补跑的分钟数
```
- 服务**在开盘时刻(≈9:31)正运行** → 后台线程自动执行一次组合调仓（`portfolio_apply()` 同一套：持仓保留/顺位补买/追高拦截/弱势与长假降仓），每日一次。
- `logs/auto_open_execute_date` 标记防同一天重复（重启不重跑）。
- 服务当时没开 → 当日不自动跑（不补历史），正合“本地项目无法 24h 挂服务”。
- 手动“一键调仓”不受影响，仍可用。

**验证**：配置默认关闭下服务正常启动、`/api/portfolio` 正常。

...
> ⚠️ **注意（别误解，我在提交信息里写错过一次）**：这次改动**没有**消除「import 即启动线程」的副作用 ——
> 三个线程只是在**文件末尾**启动，`import api.main` **依然会拉起它们**，变的只是「不再有竞态」。
> 想真正去掉导入副作用需改成 `@app.on_event("startup")`（FastAPI 起服务时才启动）；
> 本次没做，因为那是行为变更，超出修 bug 的范围。
>
> 由此有一条**排查纪律**（本次亲身踩到）：`import api.main` 会**真的触发调仓 worker**。
> 排查这个模块时不要随手 import，先读代码确认副作用。

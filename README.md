...
├── frontend/index.html       # Web 看板（Vue3 + ECharts，四个 Tab）
├── data/                     # 运行时生成（SQLite + CSV 备份）
├── paper/                    # 运行时生成（三个账户：自动纸面盘 / 模拟炒股 / 实盘¥3000）
...
### Tab 4 · 实盘炒股（¥3000）
**真金白银的记账 + 建议**：模型只给买卖**建议**，你在券商 App 人工下单，回来把成交录进来
（配置在 `config.yaml` 的 `real` 段，默认本金 **¥3000**、最多 **2 只**）。

> ⚠️ **本程序不会、也无法自动下单**：没有任何券商 API 对接，不构成投资建议；
> 它只做两件事——**给建议** + **记账**（含"没成交"留痕）。

核心是 **「这一单到底能不能成交」**（纸面漂亮的建议，实盘常因下列原因落空）：

| 判定 | 含义 |
|---|---|
| 🟢 可成交 | 委托价落在当日已触及区间、盘口有量 |
| 🔵 大概率 | 盘口深度不足（可能部分成交）、接近涨跌停 |
| 🟠 难成交 | **涨停封板买不进 / 跌停封板卖不出**、委托价高于今日最高、需改价 |
| ⚪ 已错过 | 价格已跑（相对参考价漂移 > 2%）——实盘最真实的痛 |
| 🔴 不可下单 | 非交易时段 / 停牌 / **T+1**（当日买入不可卖）/ 非整手 / 资金不足 |

同时按真实费用口径算账：**佣金万3（单笔最低 ¥5）** + 印花税 0.05%(卖) + 过户费 0.001%，
每一行都给出**含费回本涨幅**。¥3000 的硬算术：2 仓位 × 100 股 → 股价上限 ≈ ¥14，
再计最低佣金后**可买价格带约 ¥7–¥14**（低价股费用占比过高、不经济）——所以建议常常只有 0–1 只，
界面会说明原因而不是给空表。

```
建议：GET  /api/real/advice          # 可买价格带 + 买入建议 + 卖出/止损提醒（每条带成交判定）
试算：GET  /api/real/check?symbol=601600&side=buy&shares=100
记账：POST /api/real/order           # 人工成交后回报；filled=false 则只留痕
{"symbol":"601600","side":"buy","shares":100,"price":9.92,"fee":5.00,"filled":true}
误录：DELETE /api/real/trade/{id}    # 删除后按流水全量重建资金/持仓
```
...
### 🗺️ 市场状态切片验证：价量 alpha 是「条件性」的（2026-09-03）

28 的"整体超额≈0"有两种解释：因子没 alpha / 被普涨 beta 淹没。29 把验证段每交易日按「全池等权未来 20 日收益」切 弱/中/强市 三组逐组评估（脚本 `29_regime_split_validation.py`，600 池 h=20）：

| 市场状态 | 验证日 | RankIC | ICIR | 全池均值 | Top20 | **超额** | 胜率 |
|---|---|---|---|---|---|---|---|
| 整体 | 312 | +0.030 | 0.18 | +2.50% | +2.17% | −0.32% | 0.40 |
| **弱市**（未来20d<0） | 104 | **+0.118** | **0.62** | −3.34% | −1.94% | **+1.40%** | 0.59 |
| 中市 | 104 | +0.043 | 0.35 | +2.61% | +2.04% | −0.57% | 0.42 |
| **强市**（未来20d>~4.5%） | 104 | **−0.072** | **−0.77** | +8.22% | +6.42% | **−1.80%** | 0.19 |

- **结论 = 风格问题，不是因子问题**：这 51 个价量特征（低波/反转主导）的 alpha 是**状态依赖**的——
  - **弱市/下跌**：排序力最强（RankIC 0.12、ICIR 0.62），Top20 在跌市**少亏 1.4%**（−1.9% vs 全池 −3.3%）= 防御性真 alpha（低波动护身 + 超跌反弹）
  - **强市/普涨**：RankIC 转负（−0.07）、Top20 跑输全池 1.8%（0.19 胜率）——低波/反转因子在牛市的经典**负暴露**（涨不过高弹性强势票）
- **落地含义**：不应"无脑每天从 600 选 Top 并指望它任何行情都跑赢"。正确用法是**按市场状态切换**——弱市/震荡用低波+反转因子增强（有真信号）；强市切动量/顺势逻辑或接受跟随 beta（别用防御因子跟强势票硬刚）
- ⚠️ **诚实边界**：这里的分组用了「未来 20 日全池收益」（**事后 look-ahead**），是研究用诊断，**不能直接实盘择时**；落地必须用**决策日可观察的代理**（沪深300 近 N 日动量/趋势/波动率）近似"市场状态"
- 结果留存：`results/regime_split_validation.json`
...
- `quant/data/selector.py` 的 `select_daily(n, basic_topk, tech_topk, universe)`：
  ...
  - **技术面（regime 门控动态权重）**：`mom20` 20日动量 + `trend` 均线偏离 + `vol20` 低波动 + `rev60` **60日反转**（超跌加分）→ `_score_tech`。默认权重 动量25/趋势15/低波动30/反转30（600 只实证）；**regime_gating=true（默认）时按沪深300 近20日趋势切换权重**：上涨市→动量45/趋势25/低波15/反转15（追涨），震荡市→低波+反转均衡，下跌市→反转45/低波35（防守/超跌反弹）——源于「市场状态切片」实证（29号：下跌市 RankIC0.12/超额+1.4%，上涨市低波/反转负暴露）
